...
module {
  ...
  public func generateReference() : async Types.OrderReference {
  ...
  public func generateUniqueReference(orders : List.List<StorefrontTypes.Order>) : async Types.OrderReference {
    var attempt = 0;
    while (attempt < Types.MAX_GENERATION_ATTEMPTS) {
      let ref = await generateReference();
