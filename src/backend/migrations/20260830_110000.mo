...
module {
  ...
  // (20260830_100000.mo): the deployed stable shape before this change.
  type OldActor = {
  ...
  // NewActor adds the per-principal submission rate-limit state. It starts
  // empty: no principal has submitted yet, so there are no recorded timestamps.
  type NewActor = {
    products : List.List<Product>;
    orders : List.List<Order>;
    state : { var nextOrderId : Nat };
  ...
  public func migration(old : OldActor) : NewActor {
    {
      ...
      rateLimit = { var submissions = Map.empty() };
    };
  };
};
