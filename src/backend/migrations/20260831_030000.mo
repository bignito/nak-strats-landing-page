...
module {
  ...
  // (20260831_020000.mo): the deployed stable shape before this change.
  type OldActor = {
  ...
  // NewActor adds the persistent initialAdminClaimed flag. It starts false: no
  // admin has claimed initial ownership yet. The flag is carried forward by
  // every later migration so an emptied allowlist can never silently reopen the
  // one-time claim door.
  type NewActor = {
    products : List.List<Product>;
    orders : List.List<Order>;
  ...
  public func migration(old : OldActor) : NewActor {
    {
      ...
      // The persistent one-time-claim flag starts false: no admin has claimed
      // initial ownership yet. Every later migration carries it forward so an
      // emptied allowlist can never silently reopen the claim door.
      initialAdminClaimed = { var initialAdminClaimed = false };
    };
  };
};
