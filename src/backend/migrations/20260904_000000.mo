...
module {
  ...
  // (20260902_070000.mo): the deployed stable shape before this change. ----
  type ProductVariant = {
    id : Text;
    name : Text;
    size : Text;
  ...
  // NewActor adds the stable response cache for the five public read endpoints
  // and the per-principal rate-limit state for those endpoints, both seeded
  // empty. All other stable state is preserved unchanged.
  ...
  public func migration(old : OldActor) : NewActor {
    {
      ...
      publicReadRateLimit = { var calls = Map.empty() };
    };
  };
};
